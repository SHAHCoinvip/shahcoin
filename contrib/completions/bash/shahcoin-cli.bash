# bash programmable completion for shahcoin-cli(1)
...
_shahcoin_cli() {
    ...
    # save and use original argument to invoke shahcoin-cli for -help, help and RPC
    # as shahcoin-cli might not be in $PATH
    shahcoin_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($shahcoin_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_shahcoin_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
...
complete -F _shahcoin_cli shahcoin-cli
