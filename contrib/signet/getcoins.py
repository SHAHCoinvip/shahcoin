...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing shahcoin-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='shahcoin-cli', help='shahcoin-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Shahcoin address to which the faucet should send')
...
parser.add_argument('shahcoin_cli_args', nargs='*', help='Arguments to pass on to shahcoin-cli (default: -signet)')

args = parser.parse_args()

if args.shahcoin_cli_args == []:
    args.shahcoin_cli_args = ['-signet']


def shahcoin_cli(rpc_command_and_params):
    argv = [args.cmd] + args.shahcoin_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = shahcoin_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = shahcoin_cli(['getnewaddress', 'faucet', 'bech32'])
