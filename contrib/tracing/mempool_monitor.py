...
""" Example logging SHAHCOIN Core mempool events using the mempool:added,
...
def main(shahcoind_path):
    shahcoind_with_usdts = USDT(path=str(shahcoind_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    shahcoind_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    shahcoind_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    shahcoind_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    shahcoind_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[shahcoind_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/shahcoind")
        exit(1)
