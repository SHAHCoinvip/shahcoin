...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the Shahcoin community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"shahcoin_verify_binaries.{args.version}"
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the shahcoin release to download; of the format '
