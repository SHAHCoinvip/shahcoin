# CJDNS support in SHAHCOIN Core

It is possible to run SHAHCOIN Core over CJDNS, an encrypted IPv6 network that
...
done outside of SHAHCOIN Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Shahcoin network and individual nodes.
...
before it will work with your SHAHCOIN Core node. This is described in steps
...
## Run SHAHCOIN Core with CJDNS

Once you are connected to the CJDNS network, the following SHAHCOIN Core
...
When enabled, this option tells SHAHCOIN Core that it is running in an
...
IPv6 local network. This helps SHAHCOIN Core perform better address management:
...
CJDNS support was added to SHAHCOIN Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `shahcoin-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/SHAHCoinvip/shahcoin/blob/master/doc/tor.md) for
details.

## CJDNS-related information in SHAHCOIN Core

There are several ways to see your CJDNS address in SHAHCOIN Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `shahcoin-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `shahcoin-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `shahcoin-cli help getnodeaddresses` for details.
