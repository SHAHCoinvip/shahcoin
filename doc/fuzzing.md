...
```sh
$ git clone https://github.com/shahcoin/shahcoin
$ cd shahcoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/shahcoin/shahcoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/shahcoin/shahcoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/shahcoin/shahcoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/shahcoin/shahcoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`shahcoin-core/qa-assets`](https://github.com/shahcoin-core/qa-assets) repo.

To fuzz `process_message` using the [`shahcoin-core/qa-assets`](https://github.com/shahcoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/shahcoin-core/qa-assets
...
Fuzzing on a harness compiled with `--with-sanitizers=address,fuzzer,undefined` is good for finding bugs. However, the very slow execution even under libFuzzer will limit the ability to find new coverage. A good approach is to perform occasional long runs without the additional bug-detectors (configure `--with-sanitizers=fuzzer`) and then merge new inputs into a corpus as described in the qa-assets repo (https://github.com/shahcoin-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).  Patience is useful; even with improved throughput, libFuzzer may need days and 10s of millions of executions to reach deep/hard targets.
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`shahcoin-core/qa-assets`](https://github.com/shahcoin-core/qa-assets) repo.

Every single pull request submitted against the Shahcoin Core repo is automatically tested against all inputs in the [`shahcoin-core/qa-assets`](https://github.com/shahcoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Shahcoin Core more robust.
...
with certain assembly code from Shahcoin Core's code. See [developer notes on sanitizers](https://github.com/shahcoin/shahcoin/blob/master/doc/developer-notes.md#sanitizers)
...
See [this proposed patch](https://github.com/shahcoin/shahcoin/pull/22472) for more information.
