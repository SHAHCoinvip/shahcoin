...
1. The directly conflicting transactions all signal replaceability explicitly. A transaction is
   ...
   *Rationale*: See [BIP125
   explanation](https://github.com/shahcoin/bips/blob/master/bip-0125.mediawiki#motivation).
...
4. The additional fees (difference between absolute fee paid by the replacement transaction and the
   ...
   feerate is 1 shahi/vB and the replacement transaction is 500 virtual bytes total, then the
replacement pays a fee at least 500 shahi higher than the sum of the original transactions.

   *Rationale*: Try to prevent DoS attacks where an attacker causes the network to repeatedly relay
   transactions each paying a tiny additional amount in fees, e.g. just 1 shahi.
...
* Opt-in full replace-by-fee (without inherited signaling) honoured in mempool and mining as of
  **v0.12.0** ([PR 6871](https://github.com/SHAHCoinvip/shahcoin/pull/6871)).

* [BIP125](https://github.com/shahcoin/bips/blob/master/bip-0125.mediawiki) defined based on
  SHAHCOIN Core implementation.
...
* The incremental relay feerate used to calculate the required additional fees is distinct from
  ...
  ([PR #9380](https://github.com/SHAHCoinvip/shahcoin/pull/9380)).

* RBF enabled by default in the wallet GUI as of **v0.18.1** ([PR
  #11605](https://github.com/SHAHCoinvip/shahcoin/pull/11605)).

* Full replace-by-fee enabled as a configurable mempool policy as of **v24.0** ([PR
  #25353](https://github.com/SHAHCoinvip/shahcoin/pull/25353)).
