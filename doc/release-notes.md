...
- Experimental support for the v2 transport protocol defined in
  [BIP324](https://github.com/shahcoin/bips/blob/master/bip-0324.mediawiki) was added.
...
- [Miniscript](https://shahcoin.sipa.be/miniscript/) expressions can now be used in Taproot descriptors for all RPCs working with descriptors. (#27255)

- `finalizepsbt` is now able to finalize a PSBT with inputs spending [Miniscript](https://shahcoin.sipa.be/miniscript/)-compatible Taproot leaves. (#27255)
...
- `shahcoind` and `shahcoin-qt` will now raise an error on startup
 if a datadir that is being used contains a shahcoin.conf file that
 will be ignored, which can happen when a datadir= line is used in
 a shahcoin.conf file. The error message is just a diagnostic intended
 ...
 the shahcoin.conf contained in it. (#27302)
...
- A new `shahcoinconsensus_verify_script_with_spent_outputs` function is available in libconsensus which optionally accepts the spent outputs of the transaction being verified.
- A new `shahcoinconsensus_SCRIPT_FLAGS_VERIFY_TAPROOT` flag is available in libconsensus that will verify scripts with the Taproot spending rules.
...
```sh
src/shahcoin-cli -named bumpfee txid fee_rate=100
```

instead of

```sh
src/shahcoin-cli -named bumpfee txid options='{"fee_rate": 100}'
```
...
- It's now possible to use [Miniscript](https://shahcoin.sipa.be/miniscript/) inside Taproot leaves for descriptor wallets. (#27255)
