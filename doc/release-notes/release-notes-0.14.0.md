...
- #8722 `bb843ad` shahcoin-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (SHAHDrak)
...
- #9730 `33f3b21` Remove shahbitseed.xf2.org form the dns seed list (jonasschnelli)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnomshah)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnomshah)
...
- #8830 `ef0801b` test: Add option to run shahcoin-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to shahcoin-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to shahcoin-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and shahcoin-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (SHAHDrak)
...
- #9691 `fc67cd2` Init ECC context for `test_shahcoin_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change shahcoin address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` shahcoind: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow shahcoin-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` shahcoin-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to shahcoin-tx (jnewbery)
