...
`contrib/init/shahcoind.service`, it has been changed to use
`/var/lib/shahcoind` as the data directory instead of
`~shahcoin/.shahcoin`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/shahcoind` will
exist has enough space (check using `df -h /var/lib/shahcoind`), and
...
- #12490 Remove deprecated wallet rpc features from `shahcoin_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libshahcoin_server.a` (jnewbery)
...
- #13926 [Tools] shahcoin-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like shahcoind (MarcoFalke)
...
- #14521 qt, docs: Fix `shahcoin-qt -version` output formatting (hebasto)
...
- #11625 Add ShahcoinApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libshahcoin_server` (laanwj)
...
- #15154 Configure: shahcoin-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/shahcoin-wallet (Sjors)
...
- #13953 Fix deprecation in shahcoin-util-test.py (isghe)
...
- #12246 Bugfix: Only run shahcoin-tx tests when shahcoin-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_shahcoin` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_shahcoin` (MarcoFalke)
...
- #14186 shahcoin-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_shahcoin.conf` message on startup if conf file exists (leishman)
...
- #12255 Update shahcoin.service to conform to init.md (dongcarl)
...
- #14497 Add doc/shahcoin-conf.md (hebasto)
...
- #15354 Add missing `shahcoin-wallet` tool manpages (MarcoFalke)
...
- #15765 correct shahcoinconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support shahcoin folder on OSX (JimmyMow)
