...
```sh
shahcoin-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
shahcoin-cli -named createwallet mywallet load_on_startup=1
```
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching shahcoind with the configuration option
