# User-space, Statically Defined Tracing (USDT) for SHAHCOIN Core

SHAHCOIN Core includes statically defined tracepoints to allow for more
...
**Note**: SHAHCOIN Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to SHAHCOIN Core
...
Multiple tools can list the available tracepoints in a `shahcoind` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in SHAHCOIN Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/shahcoind
stap net        outbound_message 0x0000000000107c05 /src/shahcoind
stap validation block_connected  0x00000000002fb10c /src/shahcoind
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in SHAHCOIN Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/shahcoind | grep NT_STAPSDT -A 4 -B 2
