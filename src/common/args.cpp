...
const char * const SHAHCOIN_CONF_FILENAME = "shahcoin.conf";
const char * const SHAHCOIN_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //shahcoin-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", SHAHCOIN_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Shahcoin
    // macOS: ~/Library/Application Support/Shahcoin
    // Unix-like: ~/.shahcoin
#ifdef WIN32
    // Windows
