...
std::optional<ConfigError> InitConfig(ArgsManager& args, SettingsAbortFn settings_abort_fn)
{
    try {
        ...
        // shahcoin.conf file just ignores the other file.)
        const fs::path orig_datadir_path{args.GetDataDirBase()};
        const fs::path orig_config_path{AbsPathForConfigVal(args, args.GetPathArg("-conf", SHAHCOIN_CONF_FILENAME), /*net_specific=*/false)};
        ...
        // Show an error or warning if there is a shahcoin.conf file in the
        // datadir that is being ignored.
        const fs::path base_config_path = base_path / SHAHCOIN_CONF_FILENAME;
        if (fs::exists(base_config_path) && !fs::equivalent(orig_config_path, base_config_path)) {
            ...
            const std::string error = strprintf(
                ...
                fs::quoted(SHAHCOIN_CONF_FILENAME),
