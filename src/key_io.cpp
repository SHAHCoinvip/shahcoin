...
public:
    ...
    {
        std::vector<unsigned char> data = m_params.Base58Prefix(Base58Type::PUBKEY_ADDRESS);
    ...
    {
        std::vector<unsigned char> data = m_params.Base58Prefix(Base58Type::SCRIPT_ADDRESS);
        data.insert(data.end(), id.begin(), id.end());
        return EncodeBase58Check(data);
    }
...
CTxDestination DecodeDestination(const std::string& str, const CChainParams& params, std::string& error_str, std::vector<int>* error_locations)
{
    ...
    if (!is_bech32 && DecodeBase58Check(str, data, 21)) {
        // base58-encoded Shahcoin addresses.
        // Public-key-hash-addresses have version 0 (or 111 testnet).
        // The data vector contains RIPEMD160(SHA256(pubkey)), where pubkey is the serialized public key.
        const std::vector<unsigned char>& pubkey_prefix = params.Base58Prefix(Base58Type::PUBKEY_ADDRESS);
        ...
        const std::vector<unsigned char>& script_prefix = params.Base58Prefix(Base58Type::SCRIPT_ADDRESS);
...
CKey DecodeSecret(const std::string& str)
{
    ...
    if (DecodeBase58Check(str, data, 34)) {
        const std::vector<unsigned char>& privkey_prefix = Params().Base58Prefix(Base58Type::SECRET_KEY);
...
{
    assert(key.IsValid());
    std::vector<unsigned char> data = Params().Base58Prefix(Base58Type::SECRET_KEY);
...
CExtPubKey DecodeExtPubKey(const std::string& str)
{
    ...
    if (DecodeBase58Check(str, data, 78)) {
        const std::vector<unsigned char>& prefix = Params().Base58Prefix(Base58Type::EXT_PUBLIC_KEY);
...
{
    std::vector<unsigned char> data = Params().Base58Prefix(Base58Type::EXT_PUBLIC_KEY);
...
CExtKey DecodeExtKey(const std::string& str)
{
    ...
    std::vector<unsigned char> data;
    if (DecodeBase58Check(str, data, 78)) {
        const std::vector<unsigned char>& prefix = Params().Base58Prefix(Base58Type::EXT_SECRET_KEY);
        if (data.size() == BIP32_EXTKEY_SIZE + prefix.size() && std::equal(prefix.begin(), prefix.end(), data.begin())) {
            key.Decode(data.data() + prefix.size());
        }
    }
...
{
    std::vector<unsigned char> data = Params().Base58Prefix(Base58Type::EXT_SECRET_KEY);
    size_t size = data.size();
    data.resize(size + BIP32_EXTKEY_SIZE);
    key.Encode(data.data() + size);
    std::string ret = EncodeBase58Check(data);
