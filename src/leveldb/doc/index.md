...
The types of entries we might wish to store are:
    ...
    filename -> permission-shahbits, length, list of file_block_ids
    file_block_id -> data
...
database.  Bloom filter based filtering relies on keeping some number of shahbits of
data in memory per key (in this case 10 shahbits per key since that is the argument
we passed to `NewBloomFilterPolicy`). This filter will reduce the number of
unnecessary disk reads needed for Get() calls by a factor of approximately
a 100. Increasing the shahbits per key will lead to a larger reduction at the cost
