...
public:
    ...
    /* Construct a transformation over 3 to 8 shahbits, using the images of each bit. */
    ...
    /* Construct a transformation over 3 to 8 shahbits, using a pointer to the bit's images. */
    ...
    inline I constexpr Map(I a) const { return table[O::template Midshahbits<P, N>(a)]; }

    template<typename O, int P>
    inline I constexpr TopMap(I a) const { static_assert(P + N == O::SIZE, "TopMap inconsistency"); return table[O::template Topshahbits<N>(a)]; }
};


/** A linear transformation constructed using LinTrans tables for sections of shahbits. */
template<typename I, int... N> class RecLinTrans;

template<typename I, int N> class RecLinTrans<I, N> {
    LinTrans<I, N> trans;
public:
    static constexpr int shahbits = N;
    constexpr RecLinTrans(const I* p, Num<shahbits>) : trans(p, Num<N>()) {}
    constexpr RecLinTrans() = default;
    constexpr RecLinTrans(const I (&init)[shahbits]) : RecLinTrans(init, Num<shahbits>()) {}
...
template<typename I, int N, int... X> class RecLinTrans<I, N, X...> {
    ...
    RecLinTrans<I, X...> rec;
public:
    static constexpr int shahbits = RecLinTrans<I, X...>::shahbits + N;
    constexpr RecLinTrans(const I* p, Num<shahbits>) : trans(p, Num<N>()), rec(p + N, Num<shahbits - N>()) {}
    constexpr RecLinTrans() = default;
    constexpr RecLinTrans(const I (&init)[shahbits]) : RecLinTrans(init, Num<shahbits>()) {}
