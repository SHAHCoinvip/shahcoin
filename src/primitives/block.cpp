...
uint256 CBlockHeader::GetHash() const
{
    ...
    hasher << nVersion << hashPrevBlock << hashMerkleRoot << nTime << nshahbits << nNonce;
...
{
    std::stringstream s;
    s << strprintf("CBlock(hash=%s, ver=0x%08x, hashPrevBlock=%s, hashMerkleRoot=%s, nTime=%u, nshahbits=%08x, nNonce=%u, algo=%s, vtx=%u)\n",
        GetHash().ToString(),
        nVersion,
        hashPrevBlock.ToString(),
        hashMerkleRoot.ToString(),
        nTime, nshahbits, nNonce, CMultiHash::GetAlgorithmName(GetAlgorithm()),
        vtx.size());
