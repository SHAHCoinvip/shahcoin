...
{
    GUIUtil::setClipboard(ShahcoinUnits::removeSpaces(contextMenuItem->text(COLUMN_AMOUNT)));
}
...
void CoinControlDialog::updateLabels(CCoinControl& m_coin_control, WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (fWitness)
        {
            ...
            // usually, the result will be an overestimate within a couple of shahis so that the confirmation dialog ends up displaying a slightly smaller fee.
    ...
    ShahcoinUnit nDisplayUnit = ShahcoinUnit::SHAH;
    ...
    l2->setText(ShahcoinUnits::formatWithUnit(nDisplayUnit, nAmount));        // Amount
    l3->setText(ShahcoinUnits::formatWithUnit(nDisplayUnit, nPayFee));        // Fee
    l4->setText(ShahcoinUnits::formatWithUnit(nDisplayUnit, nAfterFee));      // After Fee
    l5->setText(((nBytes > 0) ? ASYMP_UTF8 : "") + QString::number(nBytes));        // Bytes
    l8->setText(ShahcoinUnits::formatWithUnit(nDisplayUnit, nChange));        // Change
    if (nPayFee > 0)
    {
    ...
    // how many shahis the estimated fee can vary per byte we guess wrong
    double dFeeVary = (nBytes != 0) ? (double)nPayFee / nBytes : 0;

    QString toolTip4 = tr("Can vary +/- %1 shahi(s) per input.").arg(dFeeVary);
...
void CoinControlDialog::updateView()
{
    ...
    ShahcoinUnit nDisplayUnit = model->getOptionsModel()->getDisplayUnit();
    ...
    for (const auto& coins : model->wallet().listCoins()) {
        ...
        for (const auto& outpair : coins.second) {
            ...
            if(ExtractDestination(out.txout.scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show shahcoin address. In tree mode, address is not shown again for direct wallet address outputs
            ...
            // amount
            itemOutput->setText(COLUMN_AMOUNT, ShahcoinUnits::format(nDisplayUnit, out.txout.nValue));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_AMOUNT, ShahcoinUnits::format(nDisplayUnit, nSum));
