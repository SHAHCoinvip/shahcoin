...
#include <qt/shahcoinaddressvalidator.h>
...
{
    std::vector<unsigned char> sourcedata = params.Base58Prefix(Base58Type::PUBKEY_ADDRESS);
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setValidator(new ShahcoinAddressEntryValidator(parent));
    widget->setCheckValidator(new ShahcoinAddressCheckValidator(parent));
}
...
bool parseShahcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no shahcoin: URI
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!ShahcoinUnits::parse(ShahcoinUnit::SHAH, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseShahcoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseShahcoinURI(uriInstance, out);
}

QString formatShahcoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("shahcoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(ShahcoinUnits::format(ShahcoinUnit::SHAH, info.amount, false, ShahcoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openShahcoinConf()
{
    ...
    /* Open shahcoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Shahcoin (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Shahcoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    ...
    return GetAutostartDir() / fs::u8path(strprintf("shahcoin-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a shahcoin.desktop file to the autostart directory:
