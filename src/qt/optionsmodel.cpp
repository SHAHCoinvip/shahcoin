...
//! Convert enabled/size values to shahcoin -prune setting.
...
//! Get pruning enabled value to show in GUI from shahcoin -prune setting.
...
//! Get pruning size value to show in GUI from shahcoin -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayShahcoinUnit")) {
        settings.setValue("DisplayShahcoinUnit", QVariant::fromValue(ShahcoinUnit::SHAH));
    }
    QVariant unit = settings.value("DisplayShahcoinUnit");
    if (unit.canConvert<ShahcoinUnit>()) {
        m_display_shahcoin_unit = unit.value<ShahcoinUnit>();
    } else {
        m_display_shahcoin_unit = ShahcoinUnit::SHAH;
        settings.setValue("DisplayShahcoinUnit", QVariant::fromValue(m_display_shahcoin_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if shahcoin.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_shahcoin_unit);
...
{
    if (new_unit.isNull() || new_unit.value<ShahcoinUnit>() == m_display_shahcoin_unit) return;
    m_display_shahcoin_unit = new_unit.value<ShahcoinUnit>();
    QSettings settings;
    settings.setValue("DisplayShahcoinUnit", QVariant::fromValue(m_display_shahcoin_unit));
    Q_EMIT displayUnitChanged(m_display_shahcoin_unit);
}
