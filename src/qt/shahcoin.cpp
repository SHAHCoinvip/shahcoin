...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<ShahcoinUnit>("ShahcoinUnit");
#else
    qRegisterMetaType<ShahcoinUnit>("ShahcoinUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. shahcoin_de.qm (shortcut "de" needs to be defined in shahcoin.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. shahcoin_de_DE.qm (shortcut "de_DE" needs to be defined in shahcoin.qrc)
...
ShahcoinApplication::ShahcoinApplication()
...
void ShahcoinApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the ShahcoinApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", ShahcoinGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void ShahcoinApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool ShahcoinApplication::createOptionsModel(bool resetSettings)
{
...
void ShahcoinApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new ShahcoinGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &ShahcoinGUI::quitRequested, this, &ShahcoinApplication::requestShutdown);
...
void ShahcoinApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void ShahcoinApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool ShahcoinApplication::baseInitialize()
{
    return node().baseInitialize();
}

void ShahcoinApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &ShahcoinApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &ShahcoinApplication::handleRunawayException);
    connect(this, &ShahcoinApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &ShahcoinApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void ShahcoinApplication::parameterSetup()
{
...
void ShahcoinApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void ShahcoinApplication::requestInitialize()
{
...
void ShahcoinApplication::requestShutdown()
{
...
void ShahcoinApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // shahcoin: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &ShahcoinGUI::handlePaymentRequest);
            connect(window, &ShahcoinGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void ShahcoinApplication::handleRunawayException(const QString &message)
{
...
void ShahcoinApplication::handleNonFatalException(const QString& message)
{
...
WId ShahcoinApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool ShahcoinApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", ShahcoinGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(shahcoin);
    Q_INIT_RESOURCE(shahcoin_locale);
    ...
    /// 6-7. Parse shahcoin.conf, determine network, switch to network specific
    ...
    // shahcoin: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
