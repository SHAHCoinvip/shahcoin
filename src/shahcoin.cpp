...
// Simple translation function for non-Qt builds
...
{
    // If Qt is used, parameters/shahcoin.conf are parsed in qt/shahcoin.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see shahcoind -h for a list of options.", argv[i])));
        }
    }
    return true;
}
