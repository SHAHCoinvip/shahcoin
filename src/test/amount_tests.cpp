...
BOOST_AUTO_TEST_CASE(GetFeeTest)
{
    ...
    BOOST_CHECK(CFeeRate(CAmount(1), 1000) == CFeeRate(1));
    // lost precision (can only resolve shahis per kB)
...
BOOST_AUTO_TEST_CASE(BinaryOperatorTest)
{
    ...
    BOOST_CHECK(b >= a);
    BOOST_CHECK(b >= b);
    // a should be 0.00000002 SHAH/kvB now
    a += a;
    BOOST_CHECK(a == b);
}
...
BOOST_AUTO_TEST_CASE(ToStringTest)
{
    CFeeRate feeRate;
    feeRate = CFeeRate(1);
    BOOST_CHECK_EQUAL(feeRate.ToString(), "0.00000001 SHAH/kvB");
    BOOST_CHECK_EQUAL(feeRate.ToString(FeeEstimateMode::SHAH_KVB), "0.00000001 SHAH/kvB");
