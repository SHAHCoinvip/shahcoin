...
static void shiftArrayRight(unsigned char* to, const unsigned char* from, unsigned int arrayLength, unsigned int shahbitsToShift)
{
    for (unsigned int T=0; T < arrayLength; ++T)
    {
        unsigned int F = (T+shahbitsToShift/8);
        if (F < arrayLength)
            to[T] = uint8_t(from[F] >> (shahbitsToShift % 8));
        else
            to[T] = 0;
        if (F + 1 < arrayLength)
            to[T] |= uint8_t(from[(F + 1)] << (8 - shahbitsToShift % 8));
    }
}

static void shiftArrayLeft(unsigned char* to, const unsigned char* from, unsigned int arrayLength, unsigned int shahbitsToShift)
{
    for (unsigned int T=0; T < arrayLength; ++T)
    {
        if (T >= shahbitsToShift/8)
        {
            unsigned int F = T-shahbitsToShift/8;
            to[T] = uint8_t(from[F] << (shahbitsToShift % 8));
            if (T >= shahbitsToShift/8+1)
                to[T] |= uint8_t(from[F - 1] >> (8 - shahbitsToShift % 8));
        }
        else {
            to[T] = 0;
