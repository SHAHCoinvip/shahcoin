...
static CBlockIndex* CreateBlockIndexWithNshahbits(uint32_t nshahbits)
{
    CBlockIndex* block_index = new CBlockIndex();
    block_index->nHeight = 46367;
    block_index->nTime = 1269211443;
    block_index->nshahbits = nshahbits;
    return block_index;
}
...
/* Given a BlockIndex with the provided nshahbits,
 * verify that the expected difficulty results.
 */
static void TestDifficulty(uint32_t nshahbits, double expected_difficulty)
{
    CBlockIndex* block_index = CreateBlockIndexWithNshahbits(nshahbits);
