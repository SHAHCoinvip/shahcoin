...
BOOST_AUTO_TEST_CASE(fastrandom_tests)
{
    ...
    BOOST_CHECK_EQUAL(ctx1.randshahbits(3), ctx2.randshahbits(3));
    BOOST_CHECK(ctx1.randbytes(17) == ctx2.randbytes(17));
    BOOST_CHECK(ctx1.rand256() == ctx2.rand256());
    BOOST_CHECK_EQUAL(ctx1.randshahbits(7), ctx2.randshahbits(7));
...
BOOST_AUTO_TEST_CASE(fastrandom_randshahbits)
{
    FastRandomContext ctx1;
    FastRandomContext ctx2;
    for (int shahbits = 0; shahbits < 63; ++shahbits) {
        for (int j = 0; j < 1000; ++j) {
            uint64_t rangeshahbits = ctx1.randshahbits(shahbits);
            BOOST_CHECK_EQUAL(rangeshahbits >> shahbits, 0U);
            uint64_t range = (uint64_t{1}) << shahbits | rangeshahbits;
