...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(result14->GetSelectedValue(), 1 * COIN);  // we should get 1 SHAH in 1 coin
        ...
        BOOST_CHECK_EQUAL(result15->GetSelectedValue(), 2 * COIN);  // we should get 2 SHAH in 1 coin
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19,8 SHAH total --> 60 × 272 WU = 16320 WU
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(0), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 SHAH total --> 10 × 272 WU = 2720 WU
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(0), 144, false, 0, true);
            }
            return available_coins;
        });
...
BOOST_AUTO_TEST_CASE(check_max_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 SHAH and 1515x 0.033 SHAH (~100.0 SHAH total) unspent outputs
        // Then tries to spend 49.5 SHAH
        // The 50.0 SHAH output should be selected, because the transaction would otherwise be too large
        ...
        BOOST_CHECK(result);
        // Verify that only the 50 SHAH UTXO was selected
...

    {
        ...
        // The actor starts with 400x 0.0625 SHAH and 2000x 0.025 SHAH (75.0 SHAH total) unspent outputs
        // Then tries to spend 49.5 SHAH
...

    {
        ...
        // The actor starts with 1515x 0.033 SHAH (49.995 SHAH total) unspent outputs
...
BOOST_AUTO_TEST_CASE(SelectCoins_effective_value_test)
{
    ...
    CoinsResult available_coins;
    {
        ...
        add_coin(available_coins, *dummyWallet, 100000); // 0.001 SHAH
    }

    CAmount target{99900}; // 0.000999 SHAH
