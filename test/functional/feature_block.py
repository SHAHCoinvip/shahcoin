...
class FullBlockTest(ShahcoinTestFramework):
    ...
    def run_test(self):
        ...
        # Extend the b26 chain to make sure shahcoind isn't accepting b26
        ...
        # Extend the b28 chain to make sure shahcoind isn't accepting b28
        ...
        # Create a transaction that spends one shahi to the p2sh_script, the rest to OP_TRUE
        ...
        # Until block is full, add tx's with 1 shahi to p2sh_script, the rest to OP_TRUE
        tx_new = None
        tx_last = tx
        ...
        # shahcoind doesn't disconnect us for sending a bloated block, but if we subsequently
        ...
        #                        \-> b68 (20)
        #
        # b68 - coinbase with an extra 10 shahi,
#       creates a tx that has 9 shahi from out[20] go to fees
#       this fails because the coinbase is trying to claim 1 shahi too much in fees
        #
        # b69 - coinbase with extra 10 shahi, and a tx that gives a 10 shahi fee
        #       this succeeds
        #
        ...
        #    Low-S and High-S values (whereas the shahcoin code has custom code which does so);
    ...
    def next_block(self, number, spend=None, additional_coinbase_value=0, script=CScript([OP_TRUE]), *, version=4):
        ...
        if spend is None:
            ...
            coinbase.vout[0].nValue += spend.vout[0].nValue - 1  # all but one shahi to fees
            coinbase.rehash()
            tx = self.create_tx(spend, 0, 1, script)  # spend 1 shahi
            self.sign_tx(tx, spend)
            tx.rehash()
