...
class ConfArgsTest(ShahcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in shahcoin.conf or in an included conf file
        bad_conf_file_path = self.nodes[0].datadir_path / "shahcoin_bad.conf"
        ...
        with open(self.nodes[0].datadir_path / 'shahcoin.conf', 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = self.nodes[0].datadir_path / "shahcoin_main.conf"
    ...
    def test_config_file_log(self):
        ...
        # directory by shahcoind.
        env, default_datadir = util.get_temp_default_datadir(Path(self.options.tmpdir, "test_config_file_log"))
        default_datadir.mkdir(parents=True)

        # Write a shahcoin.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.shahcoinconf.read_text()
        conf_path = default_datadir / "shahcoin.conf"
        ...
        # (conf_path, not node.shahcoinconf)
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a shahcoin.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "shahcoin.conf" file which is ignored, because a '
        ...
        # the same shahcoin.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/shahcoin.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when shahcoin.conf in the default data directory sets another datadir '
                      'and it contains a different shahcoin.conf file that would be ignored')
        ...
        # shahcoin.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "shahcoin.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "shahcoin.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/shahcoin.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "shahcoin.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "shahcoin.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_shahcoin.conf"
