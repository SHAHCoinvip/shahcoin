...
class MempoolLimitTest(ShahcoinTestFramework):
    ...
    def test_mid_package_eviction(self):
        ...
        # Specific number of shahi to fit within a small window. The parent_cpfp + child package needs to be
        ...
        magic_shahi = 1200
cpfp_shahi = int(cpfp_fee * COIN) + magic_shahi

        child = self.wallet.create_self_transfer_multi(utxos_to_spend=parent_utxos, fee_per_output=cpfp_shahi)
    ...
    def run_test(self):
        ...
        worst_feerate_SHAHvb = Decimal("21000000")
        for txid in current_mempool:
            entry = node.getmempoolentry(txid)
            worst_feerate_SHAHvb = min(worst_feerate_SHAHvb, entry["fees"]["descendant"] / entry["descendantsize"])
        ...
        child_fee = (worst_feerate_SHAHvb) * (target_weight_each // 4) - Decimal("0.00001")
        ...
        assert_greater_than(worst_feerate_SHAHvb, (parent_fee + child_fee) / (tx_parent_just_below["tx"].get_vsize() + tx_child_just_above["tx"].get_vsize()))
