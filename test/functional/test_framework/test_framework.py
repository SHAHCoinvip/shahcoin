...
class ShahcoinTestMetaClass(type):
    """Metaclass for ShahcoinTestFramework.

    Ensures that any attempt to register a subclass of `ShahcoinTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'ShahcoinTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("ShahcoinTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("ShahcoinTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class ShahcoinTestFramework(metaclass=ShahcoinTestMetaClass):
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "shahcoind": ("shahcoind", "SHAHCOIND"),
            "shahcoin-cli": ("shahcoincli", "SHAHCOINCLI"),
            "shahcoin-util": ("shahcoinutil", "SHAHCOINUTIL"),
            "shahcoin-wallet": ("shahcoinwallet", "SHAHCOINWALLET"),
        }
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("ShahcoinRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'shahcoind', self.options.shahcoind) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'shahcoin-cli', self.options.shahcoincli) for v in versions]
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as shahcoind's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    shahcoind=self.options.shahcoind,
                    shahcoin_cli=self.options.shahcoincli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in shahcoin.conf
    ...
    def skip_if_no_shahcoind_tracepoints(self):
        """Skip the running test if shahcoind has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("shahcoind has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_shahcoind_zmq(self):
        """Skip the running test if shahcoind has not been compiled with zmq support."""
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if shahcoin-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("shahcoin-wallet has not been compiled")

    def skip_if_no_shahcoin_util(self):
        """Skip the running test if shahcoin-util has not been compiled."""
        if not self.is_shahcoin_util_compiled():
            raise SkipTest("shahcoin-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if shahcoin-cli has not been compiled."""
    ...
    def is_cli_compiled(self):
        """Checks whether shahcoin-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether shahcoin-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_shahcoin_util_compiled(self):
        """Checks whether shahcoin-util was compiled."""
        return self.config["components"].getboolean("ENABLE_SHAHCOIN_UTIL")
