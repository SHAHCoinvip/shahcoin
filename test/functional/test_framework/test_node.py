#!/usr/bin/env python3
# Copyright (c) 2017-2022 The SHAHCOIN Core developers
...
SHAHCOIND_PROC_WAIT_TIMEOUT = 60
...
class TestNode():
    ...
    def __init__(self, i, datadir_path, *, chain, rpchost, timewait, timeout_factor, shahcoind, shahcoin_cli, coverage_dir, cwd, extra_conf=None, extra_args=None, use_cli=False, start_perf=False, use_valgrind=False, version=None, descriptors=False):
        """
        Kwargs:
        ...
        # Configuration for logging is set as command-line args rather than in the shahcoin.conf file.
        # This means that starting a shahcoind using the temp dir to debug a failed test won't
        # spam debug.log.
        self.args = [
            self.binary,
        ...
        self.cli = TestNodeCLI(shahcoin_cli, self.datadir_path)
    ...
    def __del__(self):
        # Ensure that we don't leave any shahcoind processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, *, cwd=None, stdout=None, stderr=None, env=None, **kwargs):
        ...
        # Add a new stdout and stderr file each time shahcoind is started
        if stderr is None:
        ...
        # unclean shutdown), it will get overwritten anyway by shahcoind, and
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            if self.process.poll() is not None:
                ...
                raise FailedToStartError(self._node_msg(
                    f'shahcoind exited with status {self.process.returncode} during initialization. {str_error}'))
            try:
                rpc = get_rpc_proxy(
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcpassword; shahcoind is still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        self._raise_assertion_error("Unable to connect to shahcoind after {}s".format(self.rpc_timeout))
    ...
    def wait_for_cookie_credentials(self):
        ...
        for _ in range(poll_per_s * self.rpc_timeout):
            try:
                ...
                return
            except ValueError:  # cookie file not found and no rpcuser or rpcpassword; shahcoind is still starting
    ...
    def wait_until_stopped(self, *, timeout=SHAHCOIND_PROC_WAIT_TIMEOUT, expect_error=False, **kwargs):
    ...
    def assert_start_raises_init_error(self, extra_args=None, expected_msg=None, match=ErrorMatch.FULL_TEXT, *args, **kwargs):
        ...
        with tempfile.NamedTemporaryFile(dir=self.stderr_dir, delete=False) as log_stderr, \
            ...
            try:
                ...
                self.log.debug(self._node_msg(f'shahcoind exited with status {ret} during initialization'))
                ...
                self.running = False
                self.process = None
                assert_msg = f'shahcoind should have exited within {self.rpc_timeout}s '
...
class TestNodeCLI():
    ...
    def __init__(self, binary, datadir):
        ...
        self.input = None
        self.log = logging.getLogger('TestFramework.shahcoincli')

    def __call__(self, *options, input=None):
        # TestNodeCLI is callable with shahcoin-cli command-line options
    ...
    def send_cli(self, command=None, *args, **kwargs):
        ...
        self.log.debug("Running shahcoin-cli {}".format(p_args[2:]))
