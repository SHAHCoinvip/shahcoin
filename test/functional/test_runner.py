...
`test/functional/test_framework/test_framework.py:ShahcoinTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'interface_shahcoin_cli.py --legacy-wallet',
    'interface_shahcoin_cli.py --descriptors',
...
def main():
    ...
    enable_shahcoind = config["components"].getboolean("ENABLE_SHAHCOIND")

    if not enable_shahcoind:
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if shahcoind is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "shahcoind"], stdout=subprocess.DEVNULL).returncode == 0:
